...
def fetch_jsearch_jobs(keyword: str, city: Optional[str] = None, limit: int = 12):
    ...
    try:
        ...
        for job in jobs_raw:
            jobs_cleaned.append({
                "id": f"jsearch_{job.get('job_id') or job.get('jobkey') or ''}",
...
def fetch_remotive_jobs(keyword: str = "Software Engineer", city: Optional[str] = None, limit: int = 12):
...
def get_job_detail(job_id: str, data: dict = Body(None)):
    """
    Job ID format: jsearch_<id> or remotive_<id>
    """
    if job_id.startswith("jsearch_"):
        job_key = job_id[len("jsearch_") :]
        JSEARCH_API_KEY = os.getenv("JSEARCH_API_KEY")
        JSEARCH_API_HOST = os.getenv("JSEARCH_API_HOST")
    ...
    elif job_id.startswith("remotive_"):
        job_key = job_id[len("remotive_") :]
        try:
            resp = requests.get(f"{REMOTIVE_API}", timeout=10)
            resp.raise_for_status()
            jobs = resp.json().get("jobs", [])
            for job in jobs:
                if str(job["id"]) == str(job_key):
                    return {
                        "job": {
                            "id": job_id,
                            "title": job["title"],
